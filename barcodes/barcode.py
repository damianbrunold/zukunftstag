...
infos = [
    ...
    ["-1", "STOP", "STOP", "STOP", "11000111010"]]

def get_info(b):
    for info in infos:
        if info[2] == b: return info

def get_code(value):
    result = ""
    checksum = int(get_info("START B")[0]);
    result += get_info("START B")[4]
    for i in range(len(value)):
        result += get_info(value[i])[4]
        checksum += (i + 1) * int(get_info(value[i])[0]);
    result += infos[checksum % 103][4]
    result += get_info("STOP")[4]
    result += "11"
    return result

def barcode(value, show_value=True, width=2):
    code = get_code(value)
    for i in range(len(code)):
        if code[i] == '1':
            ...
            fill_ = "#fff"
        w.create_line(100 + i * width, 250, 100 + i * width, 750, fill = fill_, width = width)
    if show_value:
        w.create_text(500, 800, text = value)

def lineal(value, width=2):
    for i in range(len(get_code(value))):
        if i % 2 == 0:
            fill_ = "#000"
        else:
            fill_ = "#fff"
        if i % 10 == 0:
            w.create_line(100 + i * width, 190, 100 + i * width, 220, fill = fill_, width = width)
            w.create_text(100 + i * width, 180, text = str(i))
        else:
            w.create_line(100 + i * width, 200, 100 + i * width, 220, fill = fill_, width = width)
